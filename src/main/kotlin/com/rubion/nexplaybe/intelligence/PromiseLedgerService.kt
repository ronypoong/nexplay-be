...
class PromiseLedgerService(
    ...
    fun extractPromises(limit: Int = DEFAULT_LIMIT): PromiseSyncSummary {
        ...
        val candidates = jdbc.query(
            ...
            WHERE NOT EXISTS (
              ...
              -- 분류에서 이미 "마케팅 잡음" 으로 판정한 글은 다시 묻지 않는다.
              -- 전체의 35%가 여기 해당한다. 그 판정에 이미 돈을 썼는데 그 정보를
              -- 쓰지 않고 같은 글을 또 보내는 것은 같은 값을 두 번 치르는 것이다.
              AND NOT EXISTS (
                  SELECT 1 FROM game_event_extraction x2
                  WHERE x2.event_id = e.id AND x2.is_marketing_noise = TRUE
              )
    ...
    private companion object {
        ...
        val SYSTEM_PROMPT = """
            ...
            대부분의 글에는 약속이 없습니다. 없으면 빈 배열을 반환하세요.

            현재형·완료형 문장은 약속이 아닙니다. "이제 부활합니다", "45초간
            지속됩니다" 는 이미 적용된 변경이거나 규칙 설명입니다.
